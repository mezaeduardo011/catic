! function(a) {
	...
	var c = function(b, c) {
		...
		this.format = d.translateFormats[e], this.isInput = this.element.is("input"), this.component = this.element.is(".color") ? this.element.find(".add-on") : !1, this.picker = a(d.template).appendTo("body").on("mousedown", a.proxy(this.mousedown, this)), this.isInput ? this.element.on({
			...
			click: a.proxy(this.show, this)
		}), ("rgba" === e || "hsla" === e) && (this.picker.addClass("alpha"), this.alpha = this.picker.find(".colorpicker-alpha")[0].style), this.component ? (this.picker.find(".colorpicker-color").hide(), this.preview = this.element.find("i")[0].style) : this.preview = this.picker.find("div:last")[0].style, this.base = this.picker.find("div:first")[0].style, this.update()
	};
	c.prototype = {
		constructor: c,
		show: function(b) {
			this.picker.show(), this.height = this.component ? this.component.outerHeight() : this.element.outerHeight(), this.place(), a(window).on("resize", a.proxy(this.place, this)), this.isInput || b && (b.stopPropagation(), b.preventDefault()), a(document).on({
				mousedown: a.proxy(this.hide, this)
		...
		update: function() {
			this.color = new b(this.isInput ? this.element.prop("value") : this.element.data("color")), this.picker.find("i").eq(0).css({
		...
		setValue: function(a) {
			this.color = new b(a), this.picker.find("i").eq(0).css({
				left: 100 * this.color.value.s,
				top: 100 - 100 * this.color.value.b
		...
		hide: function() {
			this.picker.hide(), a(window).off("resize", this.place), this.isInput ? this.element.prop("value", this.format.call(this)) : (a(document).off({
				mousedown: this.hide
		...
		mousedown: function(b) {
			...
			var c = a(b.target),
				e = c.closest("div");
			if (!e.is(".colorpicker")) {
				if (e.is(".colorpicker-saturation")) this.slider = a.extend({}, d.sliders.saturation);
				else if (e.is(".colorpicker-hue")) this.slider = a.extend({}, d.sliders.hue);
				else {
					if (!e.is(".colorpicker-alpha")) return !1;
					this.slider = a.extend({}, d.sliders.alpha)
				}
				var f = e.offset();
				this.slider.knob = e.find("i")[0].style, this.slider.left = b.pageX - f.left, this.slider.top = b.pageY - f.top, this.pointer = {
					left: b.pageX,
					top: b.pageY
				}, a(document).on({
		...
		mousemove: function(a) {
			...
			return this.slider.knob.left = b + "px", this.slider.knob.top = c + "px", this.slider.callLeft && this.color[this.slider.callLeft].call(this.color, b / 100), this.slider.callTop && this.color[this.slider.callTop].call(this.color, c / 100), this.previewColor(), this.element.trigger({
				type: "changeColor",
				color: this.color
			}), !1
		},

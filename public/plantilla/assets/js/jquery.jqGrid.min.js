		...
		return this.each(function() {
			if (!this.grid) {
				...
				if ("horizontal" === c.p.viewsortcols[1] && (X = " ui-i-asc", Y = " ui-i-desc"), N = h ? "class='ui-th-div-ie'" : "", V = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc" + X + " ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-" + f + "'></span>", V += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + Y + " ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-" + f + "'></span></span>", c.p.multiSort)
					for (Z = c.p.sortname.split(","), e = 0; e < Z.length; e++) ab = $.trim(Z[e]).split(" "), Z[e] = $.trim(ab[0]), _[e] = ab[1] ? $.trim(ab[1]) : c.p.sortorder || "asc";
				...
				W = null, d.hDiv = document.createElement("div"), $(d.hDiv).css({
					width: d.width + "px"
				}).addClass("ui-state-default ui-jqgrid-hdiv").append(lb), $(lb).append(jb), jb = null, kb && $(d.hDiv).hide(), c.p.pager && ("string" == typeof c.p.pager ? "#" !== c.p.pager.substr(0, 1) && (c.p.pager = "#" + c.p.pager) : c.p.pager = "#" + $(c.p.pager).attr("id"), $(c.p.pager).css({
					width: d.width + "px"
				}).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom").appendTo(i), kb && $(c.p.pager).hide(), E(c.p.pager, "")), c.p.cellEdit === !1 && c.p.hoverrows === !0 && $(c).bind("mouseover", function(a) {
				...
				$(c).before(d.hDiv).click(function(a) {
					...
					if (d = d === !1 || "stop" === d ? !1 : !0, d && $.isFunction(c.p.beforeSelectRow) && (d = c.p.beforeSelectRow.call(c, T[0].id, a)), "A" !== S.tagName && ("INPUT" !== S.tagName && "TEXTAREA" !== S.tagName && "OPTION" !== S.tagName && "SELECT" !== S.tagName || b) && d === !0)
						if (mb = T[0].id, nb = $.jgrid.getCellIndex(S), ob = $(S).closest("td,th").html(), $(c).triggerHandler("jqGridCellSelect", [mb, nb, ob, a]), $.isFunction(c.p.onCellSelect) && c.p.onCellSelect.call(c, mb, nb, ob, a), c.p.cellEdit === !0)
							if (c.p.multiselect && b) $(c).jqGrid("setSelection", mb, !0, a);
							else {
								mb = T[0].rowIndex;
								try {
						...
						if (b) $(c).jqGrid("setSelection", mb, !0, a);
						else {
